...
void matchDescriptors(std::vector<cv::KeyPoint> &kPtsSource, std::vector<cv::KeyPoint> &kPtsRef, cv::Mat &descSource, cv::Mat &descRef,
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string descriptorContentType, std::string matcherType, std::string selectorType)
{
    ...
    {
        if (descriptorContentType.compare("DES_HOG") == 0)
        {
            normType = cv::NORM_L2;
        }
        else {
...
void descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &descriptors, string descriptorType, double &t)
{
    ...
    {
        //cv::Ptr<cv::SiftDescriptorExtractor> dSIFT = cv::SiftDescriptorExtractor::create();
        //dSIFT->compute(img, keypoints, descriptors);
...
// Detect keypoints in image using the non traditional detectors
void detKeypointsModern(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, std::string detectorType, bool bVis, double &t)
{
...
cv::Ptr<cv::SiftFeatureDetector> detectorSIFT;
    ...
    {
        detectorSIFT = cv::SiftFeatureDetector::create();
    } 
    else 
    {
        return;
...
void detKeypointsHarris(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis, double &t)
{
